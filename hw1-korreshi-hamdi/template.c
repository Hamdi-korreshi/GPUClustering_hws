 ...
 * Andrew Sohn
 * 
 * Modified by Hamdi Korreshi Spring 2025 004
 */
...
int main(int argc, char *argv[]) {
  ...
  if (my_rank == ROOT) {
    ...
    for (int i=1; i < num_procs; i++){
      MPI_Send(A + i*my_work * n, my_work, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
    }
    printf("Sent data to other workers.\n");
  } else {
    /* Receive my_work rows of A */
    /* Use MPI_Recv */
    MPI_Recv(A, my_work, MPI_DOUBLE, ROOT, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  ...
  if (my_rank == ROOT) {
    ...
    /* Use MPI_Recv */
    for (int i = 1; i < num_procs; i++){
      MPI_Recv(C + (i * my_work), my_work, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
  } else {
    /* Send my_work rows of C to ROOT */
    /* Use MPI_Send */
    MPI_Send(C, my_work, MPI_DOUBLE, ROOT, 0, MPI_COMM_WORLD);
  }
