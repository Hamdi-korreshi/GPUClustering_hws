...
int main(int argc, char *argv[]) {
  ...
  MPI_Win winA;
  MPI_Win winC;
  MPI_Win_create(A, n * n * sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &winA);
  MPI_Win_create(C, n * n * sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &winC);
  ...
  if (my_rank == ROOT) {
    ...
    for (int i = 1; i < num_procs; i++) {
        MPI_Win_lock(MPI_LOCK_EXCLUSIVE, i, 0, winA);
        MPI_Put(A + i * my_work * n, my_work * n, MPI_DOUBLE, i, i * my_work * n, my_work * n, MPI_DOUBLE, winA);
        MPI_Win_unlock(i, winA);
    }
  } else {
  ...
  if (my_rank == ROOT) {
    ...
    /* Use MPI_Recv */
    for (int i = 1; i < num_procs; i++) {
        MPI_Win_lock(MPI_LOCK_SHARED, i, 0, winC);
        MPI_Get(C + i * my_work * n, my_work * n, MPI_DOUBLE, i, 0, my_work * n, MPI_DOUBLE, winC);
        MPI_Win_unlock(i, winC);
    } 
